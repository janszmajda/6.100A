...
class TestPS3(unittest.TestCase):
    ...
    def test_check_user_input_correct_guess(self):
        self.assertTrue(check_user_input('as', 'as'))
        self.assertTrue(check_user_input('pat', 'pat'))
        self.assertTrue(check_user_input('face', 'face'))
        self.assertTrue(check_user_input('paint', 'paint'))

    def test_check_user_input_incorrect_word(self):
        self.assertTrue(check_user_input('as', 'if'))
        self.assertTrue(check_user_input('pat', 'tip'))
        self.assertTrue(check_user_input('face', 'tarp'))
        self.assertTrue(check_user_input('paint', 'dusky'))

    def test_check_user_input_wrong_word_length(self):
        self.assertFalse(check_user_input('face', 'tarps'))
        self.assertFalse(check_user_input('moves', 'tar'))
        self.assertFalse(check_user_input('cat', ''))

    def test_check_user_input_invalid_characters(self):
        self.assertFalse(check_user_input('face', '1234'))
        self.assertFalse(check_user_input('moves', '@#$%^'))
        self.assertFalse(check_user_input('face', 'fac^'))
        self.assertFalse(check_user_input('moves', 'mov3s'))
        self.assertFalse(check_user_input('tarp', '7arp'))

    def test_get_guessed_feedback_none_correct(self):
        self.assertEqual(get_guessed_feedback('if', 'on'),
            '_ _')
        self.assertEqual(get_guessed_feedback('cat', 'men'),
            '_ _ _')
        self.assertEqual(get_guessed_feedback('face', 'plot'),
            '_ _ _ _')
        self.assertEqual(get_guessed_feedback('sassy', 'their'),
            '_ _ _ _ _')

    def test_get_guessed_feedback_incorrect_location(self):
        self.assertEqual(get_guessed_feedback('in', 'no'),
            'n _')
        self.assertEqual(get_guessed_feedback('cat', 'arc'),
            'a _ c')
        self.assertEqual(get_guessed_feedback('face', 'cent'),
            'c e _ _')
        self.assertEqual(get_guessed_feedback('sassy', 'means'),
            '_ _ a _ s')

    def test_get_guessed_feedback_some_correct(self):
        self.assertEqual(get_guessed_feedback('in', 'on'),
            '_ N')
        self.assertEqual(get_guessed_feedback('cat', 'can'),
            'C A _')
        self.assertEqual(get_guessed_feedback('face', 'fade'),
            'F A _ E')
        self.assertEqual(get_guessed_feedback('sassy', 'saint'),
            'S A _ _ _')

    def test_get_guessed_feedback_correct_word(self):
        self.assertEqual(get_guessed_feedback('in', 'in'),
            'I N')
        self.assertEqual(get_guessed_feedback('cat', 'cat'),
            'C A T')
        self.assertEqual(get_guessed_feedback('face', 'face'),
            'F A C E')
        self.assertEqual(get_guessed_feedback('sassy', 'sassy'),
            'S A S S Y')
...
Your total score is 108.'''
        with unittest.mock.patch('builtins.input',  make_input):
            ...
            try:
                ...
                if len(lines) == 3:
                    self.assertTrue("6 letters" in lines[0], "Incorrect word length")
                    self.assertTrue("6 guesses" in lines[0], "Incorrect number of guesses remaining.")
                    self.assertTrue("3 warnings" in lines[0], "Incorrect warnings remaining.")
                    self.assertTrue("c _ a _ _ _" in lines[0], "Incorrect word progress after first guess")

                    self.assertTrue("5 guesses" in lines[1], "Incorrect number of guesses remaining.")
                    self.assertTrue("a _ c o V E" in lines[1], "Incorrect word progress after second guess")

                    self.assertTrue("4 guesses" in lines[2], "Incorrect number of guesses remaining.")
                    self.assertTrue("3 tries" in lines[2], "Incorrect count of guesses")
                    self.assertTrue("Congratulations, you won!" in lines[2], "Incorrect winning criteria")
                    self.assertTrue("score" in lines[2], "Please include the word \'score\' when revealing final score!")
                    self.assertTrue("108" in lines[2], "Incorrect final score. Expected 108.")
                else:
...
_ O _ E
Alphabet HINT:
 _  b  c  _ |E| _  g  h  i  j  k  l  _  n |O| p  q  _  s  _  u  v  w  x  y  z
----------
...
_ O _ E
Alphabet HINT:
 _  b  c  _ |E| _  g  _  i  j  k  l  _  n |O| _  q  _  s  _  u  v  w  x  y  z
----------
...
_ O _ E
Alphabet HINT:
 _  b  c  _ |E| _  g  _  i  j  k  _  _  n |O| _  q  _  _  _  u  v  w  x  y  z
----------
...
Sorry, you ran out of guesses. The word was none.'''
        ...
        with unittest.mock.patch('builtins.input',  make_input):
            ...
            try:
                self.assertFalse(threw_exception, "test_play_game_short test could not be run to completion!")
                if len(lines) == 6:
                    self.assertTrue("4 letters" in lines[0], "Incorrect word length")
                    self.assertTrue("6 guesses" in lines[0], "Incorrect number of guesses remaining.")
                    self.assertTrue("3 warnings" in lines[0], "Incorrect warnings remaining.")
                    self.assertTrue("_ e _ _" in lines[0], "Incorrect word progress after first guess")

                    self.assertTrue("5 guesses" in lines[1], "Incorrect number of guesses remaining.")
                    self.assertTrue("_ O _ E" in lines[1], "Incorrect word progress after second guess")

                    self.assertTrue("4 guesses" in lines[2], "Incorrect number of guesses remaining.")
                    self.assertTrue("_ O _ E" in lines[2], "Incorrect word progress after third guess")

                    self.assertTrue("3 guesses" in lines[3], "Incorrect number of guesses remaining.")
                    self.assertTrue("_ O _ E" in lines[3], "Incorrect word progress after fourth guess")

                    self.assertTrue("2 guesses" in lines[4], "Incorrect number of guesses remaining.")
                    self.assertTrue("_ O _ E" in lines[4], "Incorrect word progress after fifth guess")

                    self.assertTrue("1 guess" in lines[5], "Incorrect number of guesses remaining.")
                    self.assertTrue("_ O _ E" in lines[5], "Incorrect word progress after sixth guess")

                    self.assertTrue("ran out of guesses" in lines[5], "Incorrect losing criteria")
                else:
                    self.assertTrue(False, "Incorrect number of output sections (defined by rows of at least 3 dashes). \
                        Compare with sample outputs for the expected placement of dashes.")
            except Exception as e:
        ...
        with unittest.mock.patch('builtins.input',  make_input):
            ...
            try:
                ...
                if len(sections) == 7:
                    ...
                    self.assertTrue("24" in sections[6], "Incorrect final score. Expected 24.")
                else:
                    self.assertTrue(False, "Incorrect number of output sections (defined by rows of at least 3 dashes). \
                        Compare with sample outputs for the expected placement of dashes.")
            except Exception as e:
...
# TODO update values depending on the number of Wordle test cases we use
point_values = {
...
if __name__ == '__main__':
    exec("import ps3 as ps3")

    sys.stdout = store
    print("Running unit tests")
    sys.stdout = MyStream(sys.stdout)
    # suite = unittest.TestSuite()
    # suite.addTest(unittest.makeSuite(TestPS3))
    suite = unittest.TestLoader().loadTestsFromTestCase(TestPS3) # makeSuite deprecated in Python 3.13
