...
class TestEntitiesUsersMovies(unittest.TestCase):
    ...
    def test_movie_get_genres(self):
        ...
        self.assertEqual(response, self.movie_genres1,
                        f"get_genres returned {response}, but {self.movie_genres1} was expected")
        response = self.movie2.get_genres()
        self.assertEqual(response, self.movie_genres2,
                        f"get_genres returned {response}, but {self.movie_genres2} was expected")
        response = self.movie3.get_genres()
        self.assertEqual(response, self.movie_genres3,
                        f"get_genres returned {response}, but {self.movie_genres3} was expected")
        response[0] = "You forgot to make a copy of the list"
        response = self.movie3.get_genres()
        self.assertEqual(response, self.movie_genres3,
                         f"get_genres returned {response}, but {self.movie_genres3} was expected.")
    ...
    def assertDictAlmostEqual(self, d1, d2, msg=None, places=7):
        ...
        for key, value in d1.items():
            if isinstance(value, tuple):
                ...
                # Lists should have the same elements but can be ordered differently
                self.assertEqual(len(d1[key]), len(d2[key]), msg=f"Lists differ in length for key {key}")
                sorted_d1 = sorted(d1[key], key=lambda x: (round(x[0], places), round(x[1], places)))
                sorted_d2 = sorted(d2[key], key=lambda x: (round(x[0], places), round(x[1], places)))

                for v1, v2 in zip(sorted_d1, sorted_d2):
                    self.assertAlmostEqual(v1[0], v2[0], places=places, msg=msg)
                    self.assertAlmostEqual(v1[1], v2[1], places=places, msg=msg)
            else:
...
class TestRecommenderComplete(unittest.TestCase):
    ...
    def test_recommender_complete_predict_ratings_large(self):
        ...
        response = {user.get_id(): rating for user, rating in response.items()}
        self.assertDictAlmostEqual(response, expected,
                         "predict_ratings does not return the correct dictionary for a large recommender from users to users")
        ...
        response = {user.get_id(): rating for user, rating in response.items()}
        self.assertDictAlmostEqual(response, expected,
                         "predict_ratings does not return the correct dictionary for a large recommender from items to items")
    ...
    def test_recommender_complete_predict_ratings_large_genre(self):
        ...
        response = {user.get_id(): rating for user, rating in response.items()}
        self.assertDictAlmostEqual(response, expected,
                         "predict_ratings with genre toggled does not return the correct dictionary for a large recommender from users to users")
        ...
        response = {user.get_id(): rating for user, rating in response.items()}
        self.assertDictAlmostEqual(response, expected,
                         "predict_ratings with genre toggled does not return the correct dictionary for a large recommender from items to items")
    ...
    def test_recommender_complete_recommend_large(self):
        ...
        self.assertEqual(response, expected,
                         "recommend does not return the correct list of movies for a large recommender from users to users, returned %s, but %s was expected" % (response, expected))
        ...
        self.assertEqual(response, expected,
                         "recommend does not return the correct list of movies for a large recommender from items to items, returned %s, but %s was expected" % (response, expected))
    ...
    def test_recommender_complete_recommend_large_genre(self):
        ...
        self.assertEqual(response, expected,
                         "recommend with genre toggled does not return the correct list of movies for a large recommender from users to users, returned %s, but %s was expected" % (response, expected))
        ...
        self.assertEqual(response, expected,
                         "recommend with genre toggled does not return the correct list of movies for a large recommender from items to items, returned %s, but %s was expected" % (response, expected))
